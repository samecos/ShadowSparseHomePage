...
- 冷灰蓝 `#658092`、雾白 `#F6F7F8`、墨色 `#191D22`
- 低饱和照片，充足留白
- 细线、极轻的玻璃质感、很少但必要的动效
...
本地开发默认口令已经写在 `.env.local`：

- 默认访客模式：页面不展示登录或后台入口
- 后台口令：`local-admin`
- 隐藏登录地址由 `HOMEPAGE_ADMIN_SLUG` 决定，形如 `http://localhost:3000/<随机字符串>`；请使用只有你知道的随机值
- HERMES Token：`local-hermes`
- 部署前请修改 `.env.local` 或平台环境变量，参考 `.env.example`
...
- 默认加载 `satellite-v4` 鸟瞰卫星影像
- 地图上方可切换 `base-v4`（Base 地图）
...
**Allowed user-agent header** 字段完全一致（区分大小写）。如果 key 同时启用了
**Allowed HTTP origins**，代理会把当前页面的 `Referer` 转发给 MapTiler；此时需要把
