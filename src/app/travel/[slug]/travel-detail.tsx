...
export function TravelDetail({
  ...
  async function addPlanItem(event: FormEvent<HTMLFormElement>) {
    ...
    const lat = coordinate(planDraft.lat);
    const lng = coordinate(planDraft.lng);
    if ((planDraft.lat.trim() && lat === undefined) || (planDraft.lng.trim() && lng === undefined)) {
    ...
    <>
          ...
          <div className={detailStyles.planningGrid}>
              ...
              <div className={detailStyles.dayList}>
                {trip.days.map((day, index) => (
                  <article className={detailStyles.day} key={day.id}>
                    <div className={detailStyles.dayHead}><span>DAY {String(index + 1).padStart(2, '0')}</span><strong>{formatDate(day.date)}</strong></div>
                    {day.title ? <p className={detailStyles.dayTitle}>{day.title}</p> : null}
                    {day.items.length > 0 ? day.items.map((item) => (
                      <div className={detailStyles.planItem} key={item.id}>
                        <span className={detailStyles.itemTime}>{item.time || '—'}</span>
                        ...
                        {item.url ? <a href={item.url} target="_blank" rel="noreferrer"><ArrowUpRight size={13} /></a> : null}
                      </div>
            ...
            <aside className={detailStyles.sideColumn}>
              ...
              <div className={detailStyles.sideBlock}>
                ...
                <div className={detailStyles.reservationList}>{trip.reservations.map((reservation) => <article className={detailStyles.reservation} key={reservation.id}><div><span className={detailStyles.reservationType}>{reservationLabels[reservation.type]}</span><strong>{reservation.title}</strong><span>{[reservation.provider, reservation.locationName].filter(Boolean).join(' · ') || '材料待补充'}</span></div><div className={detailStyles.reservationFiles}>{reservation.attachmentIds.map((id) => { const attachment = attachmentsById.get(id); return attachment ? <a href={attachmentUrl(trip.id, id)} key={id} target="_blank" rel="noreferrer">{attachment.originalName}</a> : null; })}{canEdit ? <label className={detailStyles.fileButton}><UploadIcon size={13} />上传文件<input type="file" accept="image/*,.pdf,application/pdf" onChange={(event) => handleReservationFile(reservation.id, event)} /></label> : null}</div></article>)}</div>
                {trip.reservations.length === 0 ? <p className={detailStyles.placeholder}>把确认邮件、票据或 PDF 放在对应项目下。</p> : null}
              </div>
            </aside>
          </div>

          <div className={detailStyles.mapBlock}><div className={detailStyles.blockHead}><h3>计划地图</h3><span>{mapPoints.length} POINTS</span></div><TripMap points={mapPoints} /></div>
        </section>
      ) : null}
      ...
      {phase === 'during' ? (
        <section className={detailStyles.phaseSection}>
          ...
          <div className={detailStyles.timeline}>{trip.entries.length > 0 ? [...trip.entries].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).map((entry) => <article className={detailStyles.entry} key={entry.id}><div className={detailStyles.entryTime}><span>{formatDate(entry.createdAt)}</span><strong>{formatTime(entry.createdAt)}</strong></div><div className={detailStyles.entryBody}>{entry.text ? <p>{entry.text}</p> : null}{entry.locationName ? <span className={detailStyles.entryLocation}><MapPinIcon size={13} />{entry.locationName}</span> : null}{entry.attachmentIds?.map((id) => { const attachment = attachmentsById.get(id); if (!attachment) return null; return isImageAttachment(attachment) ? <img className={detailStyles.entryImage} src={attachmentUrl(trip.id, id)} alt={attachment.originalName} key={id} /> : <a className={detailStyles.entryFile} href={attachmentUrl(trip.id, id)} target="_blank" rel="noreferrer" key={id}>{attachment.originalName} · {formatBytes(attachment.size)}</a>; })}</div></article>) : <div className={detailStyles.emptyTimeline}>还没有沿途记录。等一个值得留下来的瞬间。</div>}</div>
          <div className={detailStyles.mapBlock}><div className={detailStyles.blockHead}><h3>沿途地点</h3><span>{mapPoints.length} POINTS</span></div><TripMap points={mapPoints} /></div>
        </section>
      ) : null}
      ...
      {phase === 'after' ? (
        <section className={`${detailStyles.phaseSection} ${detailStyles.afterSection}`}>
          ...
          <div className={detailStyles.memoryList}><div className={detailStyles.blockHead}><h3>沿途记录</h3><span>EDITABLE TIMELINE</span></div>{trip.days.map((day, index) => { const entries = trip.entries.filter((entry) => entry.dayId === day.id); const items = day.items; return <article className={detailStyles.memoryDay} key={day.id}><div className={detailStyles.memoryDayHead}><span>DAY {String(index + 1).padStart(2, '0')}</span><strong>{formatDate(day.date)}</strong></div><div>{items.map((item) => <div className={detailStyles.memoryItem} key={item.id}><span>{item.time || '—'}</span><p><strong>{item.title}</strong>{item.locationName ? <small>{item.locationName}</small> : null}</p></div>)}{entries.map((entry) => <div className={detailStyles.memoryEntry} key={entry.id}><span>{formatTime(entry.createdAt)}</span><p>{entry.text || '一张照片'}{entry.locationName ? <small>{entry.locationName}</small> : null}</p></div>)}</div></article>; })}</div>
          <div className={detailStyles.mapBlock}><div className={detailStyles.blockHead}><h3>路线回看</h3><span>{mapPoints.length} POINTS</span></div><TripMap points={mapPoints} /></div>
        </section>
      ) : null}
